...
| Create Package | `/admin/packages/new` | Kinetic release form | Package metadata and first-version fields are dense | Inputs and release notes need clear grouping on mobile | Form/status patterns | Working | High — keep creation distinct from release targeting |
| Create Package Release | `/admin/packages/releases/new` | Kinetic release form | Targeting form is dense | Selected chips and controls can overflow on mobile | Company target components | Working | High — improve grouping and wrapping without changing logic |
| Create Package Version | `/admin/packages/:packageKey/versions/new` | Kinetic release form | Version metadata is compact | Long compatibility notes need readable scrolling | Form/status patterns | Working | Medium — preserve no-publish/no-assignment behavior |
...
| Release Details | `/admin/releases/:releaseId` | Kinetic operations detail | Per-company outcomes are dense | Failure details and retry actions need mobile table treatment | Table/status patterns | Working | High — make independent outcomes scannable |
