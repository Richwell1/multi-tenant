...
| Extensions Marketplace | Complete (redesign) | Search + category filter chips (All/Productivity/Finance/HR Tools/Operations), category badge + feature list per card, no-match state; per-card install/open + installed/version badges preserved | marketplace test (pending + category/search) |
| Extensions Marketplace (legacy note) | Superseded | Subtitle "Optional standalone features your company can install"; cards show description, latest/installed version, and an Open action once installed; **per-card** install pending state (only the clicked card shows "Installing…" and is disabled) | `MarketplacePage` component test; hosted browser smoke pending |
