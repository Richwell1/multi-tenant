...
| Admin UI polish — remaining | Planned | Sidebar section grouping + collapsed tooltips; top-bar profile menu; dashboard metric icons/skeleton + staggered entrance; per-page card/table/filter polish; login/logout feedback + one-toast sequencing; dialogs focus/return; responsive + a11y sweep; design-token audit; optional Framer Motion (deferred — dependency/bundle) | Phased on `feat/platform-admin-ui-polish` |
