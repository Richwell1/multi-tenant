...
export class SupabaseMarketplaceRepository implements MarketplaceRepository {
  async list(): Promise<MarketplacePackage[]> {
    ...
    const { data, error } = await client
      .from('packages')
      .select('key,name,description,category,is_active,feature_status,package_versions(version,released_at)')
    ...
    const rows = (data ?? []) as unknown as Array<{
      ...
      feature_status: 'implemented' | 'catalog_only' | null;
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        featureStatus: r.feature_status ?? 'implemented',
      };
    });
  }
